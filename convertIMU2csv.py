...
def saveTopicIMU(bag_path, topic_name, start_time, end_time, msg_num, save_path, file_type='.csv'):
    ...
    out_file.write("#timestamp [ns],Qx,Qy,Qz,Qw,w_RS_S_x [rad s^-1],w_RS_S_y [rad s^-1],w_RS_S_z [rad s^-1],a_RS_S_x [m s^-2],a_RS_S_y [m s^-2],a_RS_S_z [m s^-2]\n")
            ...
            if cur_time < start_time:
                ...
                if topic == topic_name:
                    try:
                        ...
                        # 0720 hyue modify
                        Q_x = msg.Quaternion.x
                        Q_y = msg.Quaternion.y
                        Q_z = msg.Quaternion.z
                        Q_w = msg.Quaternion.w
                        ...
                        out_file.write(timestr+","+str(Q_x)+","+str(Q_y)+","+str(Q_z)+","+str(Q_w)+str(w_x)+","+str(w_y)+","+str(w_z)+","+str(a_x)+","+str(a_y)+","+str(a_z)+"\n")
                        counter += 1
